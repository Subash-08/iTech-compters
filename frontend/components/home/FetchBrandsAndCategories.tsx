  ...
  const ProductCard: React.FC<{ product: Product }> = ({ product }) => (
    ...
    >
      <div className="aspect-w-1 aspect-h-1 bg-gray-100 overflow-hidden">
        <img 
          src={product.images.thumbnail || product.images.gallery?.[0] || '/placeholder-product.jpg'}
          alt={product.name}
          className="w-full h-48 object-cover group-hover:scale-105 transition-transform duration-300"
          onError={(e) => {
            e.currentTarget.src = 'https://placehold.co/300x300?text=Product+1';
          }}
        />
...
<Link
  key={category._id}
  to={`/products/category/${category.slug}`}  // Use slug instead of ID
...
<Link
  key={brand._id}
  to={`/products/brand/${brand.slug}`}  // Use slug instead of ID
  className="bg-gray-50 rounded-xl p-6 text-center hover:shadow-lg transition-all duration-300 group border border-gray-200 hover:border-blue-300"
>
