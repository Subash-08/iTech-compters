...
const CartItem: React.FC<CartItemProps> = ({
  ...
  const itemPrice = getItemPrice();   // tax-exclusive selling price
  const itemLink = getItemLink();
  const productSKU = getProductSKU();
  const category = getCategory();

  // Tax-inclusive display price — taxRate comes from product populated by backend
                  ...
                  <div className="space-y-1">
                    ...
                    {item.quantity > 1 && (
                      <div className="text-xs text-slate-400 font-medium">
                        ₹ {displayUnitPrice.toFixed(2)} each
                      </div>
                    )}
                    {/* Tax-inclusive label — taxRate served by backend */}
                    {taxRate > 0 && (
