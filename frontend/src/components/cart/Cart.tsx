              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                    ...
                    {productItems.length > 0 && (
                      <div className="flex justify-between text-gray-500">
                        ...
                        <span>₹{productItems.reduce((sum, item) => {
                          ...
                          const unitPrice = taxRate > 0 ? Math.round(item.price * (1 + taxRate / 100) * 100) / 100 : item.price;
                    ...
                    {preBuiltPCItems.length > 0 && (
                      <div className="flex justify-between text-gray-500">
                        ...
                        <span>₹{preBuiltPCItems.reduce((sum, item) => {
                          const taxRate = item.preBuiltPC?.taxRate ?? 0;
                          const unitPrice = taxRate > 0 ? Math.round(item.price * (1 + taxRate / 100) * 100) / 100 : item.price;
                          return sum + (item.quantity * unitPrice);
                        }, 0).toLocaleString()}</span>
                      </div>
                ...
                <div className="border-t border-gray-100 pt-4 mb-6">
                  <div className="flex justify-between items-end">
                    ...
                    <span className="text-2xl font-extrabold text-gray-900">₹{Math.round(cartTotal).toLocaleString('en-IN')}</span>
                  </div>
