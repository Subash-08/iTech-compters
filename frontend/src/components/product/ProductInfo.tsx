...
const ProductInfo: React.FC<ProductInfoProps> = ({
  ...
  // --- Parity Logic with ProductCard ---
  const taxRate = productData.taxRate || 0;

  const getSellingPrice = () => {
    if (selectedVariant) return selectedVariant.offerPrice || selectedVariant.price || 0;
    return productData.offerPrice || productData.effectivePrice || productData.basePrice || 0;
  };

  const getDisplayMrp = () => {
    if (selectedVariant) return selectedVariant.mrp || selectedVariant.price || 0;
    return productData.mrp || productData.effectivePrice || 0;
  };

  // Tax-inclusive price shown to the customer (selling price + GST)
  const sellingPrice = getSellingPrice();
  const displayOfferPrice = taxRate > 0
    ? Math.round(sellingPrice * (1 + taxRate / 100))
    : sellingPrice;

  // MRP is ALREADY tax-inclusive in the database
  const displayMrp = getDisplayMrp();

  // Final boolean and percentage matching ProductCard
  const isDiscounted = displayMrp > displayOfferPrice && displayOfferPrice > 0;
  const savings = isDiscounted
    ? Math.round(((displayMrp - displayOfferPrice) / displayMrp) * 100)
    : 0;

  // Stock determination
  const canAddToCart = selectedVariant ? (selectedVariant.stockQuantity || 0) > 0 : (productData.stockQuantity || 0) > 0;
  ...
  // This ensures the wishlist gets the actual selling price
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-baseline">
          <span className="text-3xl font-bold text-gray-900">
            ₹{displayOfferPrice.toLocaleString('en-IN')}
          </span>

          {/* MRP as raw strikethrough — GST only applies to selling price */}
          {isDiscounted && (
            <>
        ...
        {savings > 0 && (
          <p className="text-green-600 text-sm font-medium">
            You save ₹{(displayMrp - displayOfferPrice).toLocaleString('en-IN')} ({savings}% off)
          </p>
        )}
        ...
        <div className="text-gray-500 text-sm">
          {taxRate > 0
            ? `Inclusive of ${Math.round(taxRate)}% GST`
