...
const ProductForm: React.FC<ProductFormProps> = ({
  ...
  // 🆕 Video State via user request
  const [videoFiles, setVideoFiles] = useState<File[]>([]);
  const [videoThumbnailFiles, setVideoThumbnailFiles] = useState<File[]>([]);
  ...
  useEffect(() => {
    if (initialData && !isInitialized) {
      ...
      if (initialData.videos && Array.isArray(initialData.videos)) {
        setExistingVideos(initialData.videos.map((v: any) => v.public_id));
      }
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (videoFiles.length > 5) {
        throw new Error("Maximum 5 new video uploads allowed.");
      }
      if (videoFiles.length !== videoThumbnailFiles.length) {
        throw new Error("Each video must have a thumbnail.");
      }
      ...
      videoThumbnailFiles.forEach(file => {
  ...
  const sections = [
    ...
    { id: 'videos', label: 'Videos', component: VideosSection },
              ...
              <ActiveComponent
                {...({
                  ...
                  ...(activeSection === "videos"
                    ? {
                      onFilesChange: (files: { videos: File[], videoThumbnails: File[] }) => {
                        setVideoFiles(files.videos);
                        setVideoThumbnailFiles(files.videoThumbnails);
                      },
                      videoFiles,
                      videoThumbnailFiles,
                      existingVideos,
